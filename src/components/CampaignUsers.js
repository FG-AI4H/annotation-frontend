...
class CampaignUsers extends Component {
  ...
  constructor(props) {
    ...
    this.handleAnnotatorFilterChange =
      this.handleAnnotatorFilterChange.bind(this);
    ...
    Auth.currentAuthenticatedUser({
      ...
      .then((response) => {
        ...
        userClient.fetchUserList().then((response) => {
          if (response?.data) {
            this.state.annotatorDtos = this.state.campaign.annotators.map(
              (id) => response.data.filter((u) => u.id === id)[0]
            );
            this.state.reviewerDtos = this.state.campaign.reviewers.map(
              (id) => response.data.filter((u) => u.id === id)[0]
            );
            this.state.supervisorDtos = this.state.campaign.supervisors.map(
              (id) => response.data.filter((u) => u.id === id)[0]
            );
            ...
            this.setState({
              availableUsers: response.data
                .filter(
                  (i) => !this.state.annotatorDtos?.some((a) => a.id === i.id)
                )
                .filter(
                  (i) => !this.state.reviewerDtos?.some((a) => a.id === i.id)
                )
                .filter(
                  (i) => !this.state.supervisorDtos?.some((a) => a.id === i.id)
                ),
              isLoading: false,
            });
          }
  ...
  handleFindAnnotators(event) {
    let updatedUsers = [...this.state.availableUsers].filter(
      (i) =>
        i.years_in_practice >= this.state.annotatorFilter['years_in_practice']
    );
  ...
  removeSelectUser(user) {
    ...
    let updatedUsers = [...this.state.campaign.annotators].filter(
      (i) => i !== user.id
    );
    this.state.campaign.annotators = updatedUsers;
    this.state.annotatorDtos = [...this.state.annotatorDtos].filter(
      (i) => i.id !== user.id
    );
    this.setState({
  ...
  selectUser(user) {
    ...
    let availableUsers = [...this.state.availableUsers].filter(
      (i) => i.id !== user.id
    );
  ...
  removeSelectReviewer(user) {
    this.state.availableUsers.push(user);
    let updatedUsers = [...this.state.campaign.reviewers].filter(
      (i) => i !== user.id
    );
    this.state.campaign.reviewers = updatedUsers;
    this.state.reviewerDtos = [...this.state.reviewerDtos].filter(
      (i) => i.id !== user.id
    );
    this.setState({
      item: this.state.campaign,
  ...
  removeSelectSupervisor(user) {
    this.state.availableUsers.push(user);
    let updatedUsers = [...this.state.campaign.supervisors].filter(
      (i) => i.id !== user.id
    );
    this.state.campaign.supervisors = updatedUsers;
    this.state.supervisorDtos = [...this.state.supervisorDtos].filter(
      (i) => i.id !== user.id
    );
    this.setState({
      item: this.state.campaign,
  ...
  render() {
    ...
    const availableAnnotatorList = availableUsers
      .filter((user) => user.annotator_role?.id !== undefined)
      .map((user) => {
        return (
          <TableRow
            key={user.id}
          ...
          >
            ...
            <TableCell style={{ whiteSpace: 'nowrap' }} align={'right'}>
              {user.annotator_role.self_assessment}
            </TableCell>
            <TableCell>
          ...
          <TableCell>
            <Stack direction='row' spacing={2}>
              <Button
                size='small'
                color='error'
                onClick={() => this.removeSelectUser(user)}
              >
                Remove
              </Button>
            </Stack>
    ...
    const availableReviewerList = availableUsers
      .filter((user) => user.reviewer_role?.id !== undefined)
      .map((user) => {
        return (
          <TableRow
            key={user.id}
          ...
          >
            ...
            <TableCell style={{ whiteSpace: 'nowrap' }} align={'right'}>
              {user.reviewer_role.self_assessment}
            </TableCell>
            <TableCell>
              <Stack direction='row' spacing={2}>
                <Button
                  size='small'
                  color='success'
                  onClick={() => this.selectReviewer(user)}
                >
                  Select
                </Button>
              </Stack>
          ...
          <TableCell>
            <Stack direction='row' spacing={2}>
              <Button
                size='small'
                color='error'
                onClick={() => this.removeSelectReviewer(user)}
              >
                Remove
              </Button>
            </Stack>
    ...
    const availableSupervisorList = availableUsers
      .filter((user) => user.supervisor_role?.id !== undefined)
      .map((user) => {
        return (
          <TableRow
            key={user.id}
          ...
          >
            ...
            <TableCell style={{ whiteSpace: 'nowrap' }} align={'right'}>
              {user.supervisor_role.self_assessment}
            </TableCell>
            <TableCell>
              <Stack direction='row' spacing={2}>
                <Button
                  size='small'
                  color='success'
                  onClick={() => this.selectSupervisor(user)}
                >
                  Select
                </Button>
              </Stack>
          ...
          <TableCell>
            <Stack direction='row' spacing={2}>
              <Button
                size='small'
                color='error'
                onClick={() => this.removeSelectSupervisor(user)}
              >
                Remove
              </Button>
            </Stack>
            ...
            <Grid container>
              <Grid item xs={12}>
                ...
                <Stack direction='row' spacing={2}>
                  <Button
                    color='primary'
                    onClick={() => this.handleFindAnnotators()}
                  >
                    Search
                  </Button>{' '}
                  <Button
                    color='primary'
                    onClick={() => this.handleResetAnnotators()}
                  >
                    Reset
                  </Button>
                </Stack>
              ...
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{availableSupervisorList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{supervisorList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
            </Grid>
              ...
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{availableAnnotatorList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{annotatorList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
            </Grid>
              ...
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{availableReviewerList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
              <Grid item xs={6}>
                ...
                >
                  <Table aria-label='simple table' stickyHeader>
                    ...
                    <TableBody>{reviewerList}</TableBody>
                  </Table>
                </TableContainer>
              </Grid>
            </Grid>
