  ...
  async function remove(id) {
    Auth.currentAuthenticatedUser({
      ...
      .then((currentUser) => {
        ...
        client.removeDataAccessRequest(id).then((_response) => {
          let updatedDataAccessRequests = [...dataAccessRequests].filter(
            (i) => i.id !== id
          );
          ...
          <TableBody>
            {dataAccessRequests?.map((request, index) => (
              <TableRow key={request.id}>
                <TableCell>
                  <Link href='#' onClick={() => viewRequest(request)}>
                    {request.dataset_name}
                  </Link>
                </TableCell>
                <TableCell>
                  {new Date(Date.parse(request.request_date)).toLocaleString(
                    navigator.language
                  )}
                </TableCell>
                <TableCell>
                  {owner ? request.requester_name : request.data_owner_name}
                </TableCell>
