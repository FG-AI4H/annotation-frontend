  ...
  async function remove(id) {
    Auth.currentAuthenticatedUser({
      ...
      .then((currentUser) => {
        ...
        client.removeDataset(id).then((_response) => {
          ...
          <TableBody>
            {datasets?.map((dataset) => (
              <TableRow key={dataset.id}>
                <TableCell>
                  {dataset.linked ? <LinkIcon /> : <StorageIcon />}
                </TableCell>
                <TableCell>
                  <Link href='#' onClick={() => viewDataset(dataset.id)}>
                    {dataset.name}
                  </Link>
                </TableCell>
                <TableCell>
                  {new Date(Date.parse(dataset.created_at)).toLocaleString(
                    navigator.language
                  )}
                </TableCell>
                <TableCell>
                  {dataset.linked ? dataset.request_status : ''}
                </TableCell>
                <TableCell>
                  <Stack
                    direction={'row'}
