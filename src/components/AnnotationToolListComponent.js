...
const AnnotationToolListComponent = (props) => {
    ...
    useEffect(() => {
        ...
        Auth.currentAuthenticatedUser({
            ...
            adminCLient.fetchAnnotationToolList()
                .then(
                    response => {
                        if (response?.data) {
    ...
    const toolList = annotationToolList?.map(tool => {
        return <TableRow key={tool.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
            <TableCell style={{whiteSpace: 'nowrap'}}>{tool.name}</TableCell>
            <TableCell style={{whiteSpace: 'nowrap'}}>{tool.editor}</TableCell>
            <TableCell>
                <Stack direction={"row"} spacing={2} justifyContent="flex-end">
                    <Button component={RouterLink} size="small" to={"/annotation_tool/" + tool.id}>Edit</Button>
                </Stack>
