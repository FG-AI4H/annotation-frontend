  ...
  async remove(id) {
    Auth.currentAuthenticatedUser({
      ...
      .then((currentUser) => {
        ...
        client.removeTask(id).then((_response) => {
    ...
    const taskList = tasks?.map((task) => {
      return (
        ...
        >
          ...
          {!me && <TableCell>{task.assignee_username}</TableCell>}
          {me && <TableCell>{task.samples.map((s) => s.title)}</TableCell>}
            ...
            <Stack direction={'row'} spacing={2} justifyContent='flex-end'>
              ...
              {task.campaign_status === 'RUNNING' && (
                <Button
                  ...
                  onClick={() =>
                    window.open(
                      'https://dev.visian.org/?origin=who&taskId=' + task.id,
                      '_blank'
                    )
                  }
                >
      ...
      <div>
        ...
        <Container maxWidth='xl' sx={{ mt: 5 }}>
          <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
            ...
            {!me && (
              <Button component={RouterLink} color='success' to={'/tasks/new'}>
                Add Task
              </Button>
            )}
          </Box>
            ...
            <Table sx={{ minWidth: 650 }} aria-label='simple table'>
              <TableHead>
                <TableRow>
                  ...
                  {!me && <TableCell>Annotator</TableCell>}
                  {me && <TableCell>Samples</TableCell>}
          ...
          <Button component={RouterLink} color='secondary' to={'/annotation'}>
            Back
          </Button>
        </Container>
      </div>
