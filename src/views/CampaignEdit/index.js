    ...
    Auth.currentAuthenticatedUser({
      ...
      .then((response) => {
        if (params.id !== 'new') {
          ...
          campaignClient.fetchCampaignById(params.id).then((response) => {
            setIsLoading(false);
  ...
  async function generateTasks(event) {
    ...
    Auth.currentAuthenticatedUser({
      ...
      .then((response) => {
        ...
        client.generateTasks(item.id).then((response) => setUpdated(true));
      })
      .catch((err) => console.log(err));
  }
  ...
  async function startCampaign(event) {
    ...
    Auth.currentAuthenticatedUser({
      ...
      .then((response) => {
        ...
        client.startCampaign(item.id).then((response) => setUpdated(true));
        item.status = 'RUNNING';
      })
      .catch((err) => console.log(err));
  }
      ...
      <Container maxWidth='xl' sx={{ mt: 5 }}>
        ...
        {item.id && (
          <Box sx={{ width: '100%' }}>
                ...
                {item.status === 'DRAFT' && (
                  <>
                    ...
                    <Stack direction='row' spacing={2}>
                      <Button color='info' onClick={(e) => generateTasks(e)}>
                        Generate tasks
                      </Button>
                      <Button color='success' onClick={(e) => startCampaign(e)}>
                        Start campaign
                      </Button>
                    </Stack>
                  </>
            ...
            <Button component={RouterLink} color='secondary' to='/campaigns'>
              Back
            </Button>
          </Box>
        )}
