    ...
    Auth.currentAuthenticatedUser({
      ...
      .then((response) => {
        if (params.id !== 'new') {
          ...
          client.fetchTaskById(params.id).then((response) => {
      ...
      <Container className={'pt-5'}>
        ...
        <FormGroup sx={{ mt: 5 }}>
          <Stack direction='row' spacing={2}>
            ...
            <Button component={RouterLink} color='secondary' to={'/tasks'}>
              Cancel
            </Button>
            {params.id !== 'new' && item.campaign_status === 'RUNNING' && (
              <Button
                ...
                onClick={() =>
                  window.open(
                    'https://dev.visian.org/?origin=who&taskId=' + item.id,
                    '_blank'
                  )
                }
              >
